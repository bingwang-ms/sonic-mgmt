...
NULL_ROUTE_HELPER_SRC = "acl/null_route/null_route_helper"
NULL_ROUTE_HELPER_DST = "/usr/bin/" + os.path.basename(NULL_ROUTE_HELPER_SRC)
...
@pytest.fixture(scope="module")
def apply_exo_acl_rules(rand_selected_dut, create_acl_table):
    """
...
@pytest.fixture(scope="module")
def setup_dut(rand_selected_dut, apply_exo_acl_rules):
    """
    Copy null_route_helper to DUT for testing.
    This function will be deprecated after null_route_helper is checked in in sonic-utility
    """
    rand_selected_dut.copy(src=NULL_ROUTE_HELPER_SRC, dest=NULL_ROUTE_HELPER_DST)
    rand_selected_dut.shell("chmod +x {}".format(NULL_ROUTE_HELPER_DST))
    yield
    rand_selected_dut.file(path=NULL_ROUTE_HELPER_DST, state="absent")
...
def test_null_route_helper(rand_selected_dut, tbinfo, ptfadapter, setup_dut, setup_ptf):
    """
    ...
    for test_item in TEST_DATA:
        ...
        if action != "":
            rand_selected_dut.shell(NULL_ROUTE_HELPER_DST + " " + action)
